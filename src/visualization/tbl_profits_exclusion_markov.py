...
sympy_expr=get_sympy_expr_for_e_min_markov_blocks()[0]
...
for p_E in O_a:
    ...
    for p_m in O_m:
        ...
        if min_cost is not None:
            if min_cost < 0:
                min_cost = 0
            profit_attack_with_m = EV_fork_markov_blocks(p_B=p_B,
                                                          p_E=p_E,
                                                          p_V=p_V,
                                                          p_I=p_I,
                                                          p_m=p_m,
...
\\caption{{Comparison of minimum bribe required per block $ \\epsilon $ for $\\rho_{{\\textit{{fork bl.}}}}>\\rho_{{\\textit{{main bl.}}}}$ with probabilities calculated using our Markov chain.
The axis iterate the hashrate of an individual miner $ p_m $, and other attackers $ p_\\mathcal{{E}} $.
The table also shows the expected reward of miner $ m $, if $ p_m $ would be directed towards the attack chain $                \\rho'=\\rho_{{\\textit{{fork bl.}}}} $, as well as the expected reward $ \\rho=\\rho_{{\\textit{{main bl.}}}} $, if $ p_m $ would be directed towards the main chain.
All attacks start with a disadvantage of $ \\kbe = {:d} $, a duration of $ N = {:d} $ and the following configuration of the Markov chain $ \\dkra = {:d}, \\dkla = {:d}, \\eta_{{attack}} = {:d}, \\eta_{{main}} = {:d}, p_\\mathcal{{V}} = {:f}$.}}
\\label{{tab:txexclib}}
\\end{{table*}}
